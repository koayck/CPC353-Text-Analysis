...
for idx, tokenized_text in enumerate(tokenized_texts):
    ...
    with open(f'./pos-tagged-txt/{current_file_name}.txt', 'w', encoding="utf-8") as tagged_file:
        for sentence in tokenized_text:
            ...
            # Format and write the tagged sentence to the file in the desired format (word_POS)
            formatted_tags = [f"{word}_{tag}" for word, tag in tagged]  # Join word and tag with "_"
            tagged_file.write(' '.join(formatted_tags) + '\n')  # Write all words in the sentence in the format: word_POS
